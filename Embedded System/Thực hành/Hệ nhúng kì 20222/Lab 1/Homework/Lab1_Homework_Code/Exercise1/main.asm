...
;	main.c:45: for (j = 0; j < 7; j++) {
	...
	mov	(_j + 1),a
00103$:
;	main.c:46: led = ~hieu_ung_1;
	mov	a,r7
	cpl	a
	mov	_P1,a
;	main.c:47: delay_ms(delayms);
	mov	r3,_delayms
	mov	a,(_delayms + 1)
	mov	r4,a
	rlc	a
	...
	lcall	_delay_ms
	pop	ar7
;	main.c:48: hieu_ung_1 = hieu_ung_1 << 1;
	mov	ar6,r7
	mov	a,r6
	add	a,r6
	mov	r7,a
...
00121$:
	...
	subb	a,#0x80
	jc	00103$
;	main.c:52: hieu_ung_1 = 0xC0;  // Cho 2 led cuối sáng
	mov	r7,#0xc0
;	main.c:53: for (j = 0; j < 7; j++) {
	clr	a
	mov	_j,a
	mov	(_j + 1),a
00105$:
;	main.c:54: led = ~hieu_ung_1;
	mov	a,r7
	cpl	a
	mov	_P1,a
;	main.c:55: delay_ms(delayms);
	mov	r3,_delayms
	mov	a,(_delayms + 1)
	mov	r4,a
	rlc	a
	...
	lcall	_delay_ms
	pop	ar7
;	main.c:56: hieu_ung_1 = hieu_ung_1 >> 1;
	mov	a,r7
	clr	c
	rrc	a
	mov	r7,a
...
;------------------------------------------------------------
;	main.c:61: void hieu_ung_2() {
;	-----------------------------------------
;	 function hieu_ung_2
;	-----------------------------------------
_hieu_ung_2:
;	main.c:62: led = 0xFF;  // Tắt tất cả led
	mov	_P1,#0xff
;	main.c:63: delay_ms(delayms);
	mov	r4,_delayms
	mov	a,(_delayms + 1)
	mov	r5,a
	rlc	a
	...
	mov	b,r6
	lcall	_delay_ms
;	main.c:66: unsigned char giu_hieu_ung = 0x00;
;	main.c:71: for (j = 0; j < 8; j++) {
	clr	a
	mov	r7,a
	mov	_j,a
	mov	(_j + 1),a
...
00109$:
;	main.c:72: unsigned char hieu_ung_2 = 0x01;  // bật sáng led đầu tiên
	mov	r4,#0x01
;	main.c:74: for (k = 0; k < countled; k++) {
	clr	a
	mov	_k,a
	mov	(_k + 1),a
...
00107$:
	...
	subb	a,b
	jnc	00103$
;	main.c:76: led = ~(hieu_ung_2 | giu_hieu_ung);
	mov	a,r7
	orl	a,r4
	cpl	a
	mov	_P1,a
;	main.c:77: delay_ms(delayms);
	mov	r0,_delayms
	mov	a,(_delayms + 1)
	mov	r1,a
	rlc	a
	...
	pop	ar4
	pop	ar7
;	main.c:80: if (k != countled - 1) hieu_ung_2 = hieu_ung_2 << 1;
	mov	a,r2
	cjne	a,_k,00141$
	mov	a,r3
	cjne	a,(_k + 1),00141$
...
;	main.c:74: for (k = 0; k < countled; k++) {
	...
	sjmp	00107$
00103$:
;	main.c:83: giu_hieu_ung = hieu_ung_2 | giu_hieu_ung;
	mov	a,r4
	orl	ar7,a
;	main.c:84: countled--;  // Giảm vòng lặp dịch led
	dec	_hieu_ung_2_countled_65537_13
	mov	a,#0xff
	cjne	a,_hieu_ung_2_countled_65537_13,00143$
...
00144$:
	...
	ljmp	00109$
00145$:
;	main.c:88: for (j = 0; j < 8; j++) {
	clr	a
	mov	_j,a
	mov	(_j + 1),a
00111$:
;	main.c:90: led = ~giu_hieu_ung;
	mov	a,r7
	cpl	a
	mov	_P1,a
;	main.c:91: delay_ms(delayms);
	mov	r3,_delayms
	mov	a,(_delayms + 1)
	mov	r4,a
	rlc	a
	...
	lcall	_delay_ms
	pop	ar7
;	main.c:93: giu_hieu_ung = giu_hieu_ung >> 1;
	mov	a,r7
	clr	c
	rrc	a
	mov	r7,a
...
;------------------------------------------------------------
;	main.c:98: void hieu_ung_3() {
;	-----------------------------------------
;	 function hieu_ung_3
;	-----------------------------------------
_hieu_ung_3:
;	main.c:99: led = 0xFF;  // Tắt tất cả các led
	mov	_P1,#0xff
;	main.c:100: delay_ms(delayms);
	mov	r4,_delayms
	mov	a,(_delayms + 1)
	mov	r5,a
	rlc	a
	...
	mov	b,r6
	lcall	_delay_ms
;	main.c:102: unsigned char hieu_ung_3_1 = 0x01;  // Bật led đầu tiên
	mov	r7,#0x01
;	main.c:103: unsigned char hieu_ung_3_2 = 0x80;  // Bật led cuối cùng
	mov	r6,#0x80
;	main.c:104: for (j = 0; j < 8; j++) {
	clr	a
	mov	_j,a
	mov	(_j + 1),a
00104$:
;	main.c:105: led = ~(hieu_ung_3_1 | hieu_ung_3_2);
	mov	a,r6
	orl	a,r7
	mov	r5,a
	cpl	a
	mov	_P1,a
;	main.c:108: if (hieu_ung_3_1 != 0x08) delay_ms(delayms);
	cjne	r7,#0x08,00116$
	sjmp	00102$
00116$:
	mov	r2,_delayms
	...
	pop	ar7
00102$:
;	main.c:110: hieu_ung_3_1 = hieu_ung_3_1 << 1;
	mov	ar5,r7
	mov	a,r5
	add	a,r5
	mov	r7,a
;	main.c:111: hieu_ung_3_2 = hieu_ung_3_2 >> 1;
	mov	a,r6
	clr	c
	rrc	a
	mov	r6,a
...
;	main.c:116: while (1) {
00102$:
;	main.c:118: hieu_ung_1();
	lcall	_hieu_ung_1
;	main.c:119: hieu_ung_1();
	lcall	_hieu_ung_1
;	main.c:120: hieu_ung_2();
	lcall	_hieu_ung_2
;	main.c:121: hieu_ung_2();
	lcall	_hieu_ung_2
;	main.c:122: hieu_ung_3();
	lcall	_hieu_ung_3
;	main.c:123: hieu_ung_3();
	lcall	_hieu_ung_3
;	main.c:125: }
	sjmp	00102$
	.area CSEG    (CODE)
